...
public class RiskCalculatorTest extends TestCase {
	...
	public void off_testFormulaCffSimpleTarget() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 2,
						1, "", "", "", "", "").features();
				while(iter.hasNext()) {
	...
	public void testFormulaCffHuman() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 2, 98, "", "", "", "", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					assertEquals(0.3, feature.getAttribute("rischio1"));
	...
	public void aatestFormulaCffAll() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 2, 100, "", "", "", "", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					assertEquals(0.3, feature.getAttribute("rischio1"));
	...
	public void off_testFormulaPadr() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 20, 0, "9", "", "", "", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
	...
	public void off_testFormulaPis() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 22, 0, "", "", "", "", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();					
					assertNotNull(feature);
					count++;
	...
	public void ddtestFormulaPsc() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 16, 0, "1,4", "4,5", "0,1", "", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					count++;
	...
	public void eetestFormulaSantropica() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 36, 1, "", "4,5", "", "1,2,3,4", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					count++;
	...
	public void fftestFormulaSambientale() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 1, 37, 10, "", "4,5", "", "5", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					count++;
	...
	public void aatestFormulaE() throws IOException, SQLException {
		...
		if(riskCalculator != null && dataStoreName != null && dataStore != null) {
			...
			SimpleFeatureIterator iter = null;
			try {
				iter = riskCalculator.execute(featColl, dataStoreName, null, null, null, 32, 1, 1, "9", "4,5", "0,1", "1,2,3,4", "").features();
				while(iter.hasNext()) {
					SimpleFeature feature = iter.next();
					assertNotNull(feature);
					count++;
