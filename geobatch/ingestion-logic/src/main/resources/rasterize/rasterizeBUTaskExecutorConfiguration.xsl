...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/GdalRasterize">-a_srs "EPSG:32632" -co "TILED=YES" -co "BLOCKXSIZE=512" -co "BLOCKYSIZE=512" -a_nodata 0 -ot Float32 -a_nodata "-1.0" -tr 10 10<xsl:apply-templates select="@*|node()" /></xsl:template>
	
	<xsl:template match="@*|node()">
		<xsl:apply-templates select="@*|node()" />
	</xsl:template>
	
	<xsl:template match="attributeToNormalize">
	...
	<xsl:template match="shapefilepath">
		<xsl:value-of select="concat(' ','_baseOutputPath_',substring(//shapefilename,1,2),'/',//shapefilename,'_normalized.shp')" />
		<xsl:apply-templates select="@*|node()" />
	</xsl:template>
